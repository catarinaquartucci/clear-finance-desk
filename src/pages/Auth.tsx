import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { lovable } from "@/integrations/lovable/index";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { toast } from "sonner";
import { useAuth } from "@/contexts/AuthContext";
import { Logo } from "@/components/Brand/Logo";
import { ExternalLink } from "lucide-react";

const isInIframe = () => {
  try {
    return window.self !== window.top;
  } catch {
    return true;
  }
};

const Auth = () => {
  const navigate = useNavigate();
  const { user, isAdmin, loading: authLoading } = useAuth();
  const [showFallback, setShowFallback] = useState(false);
  const [loginLoading, setLoginLoading] = useState(false);

  useEffect(() => {
    if (!authLoading && user) {
      navigate(isAdmin ? "/admin" : "/solicitacoes");
    }
  }, [user, isAdmin, authLoading, navigate]);

  const handleGoogleLogin = async () => {
    setLoginLoading(true);
    setShowFallback(false);
    try {
      const { error } = await lovable.auth.signInWithOAuth("google", {
        redirect_uri: window.location.origin,
        extraParams: {
          prompt: "select_account",
        },
      });
      if (error) {
        const msg = error.message || String(error);
        console.error("[Auth] OAuth error:", msg, "| iframe:", isInIframe());

        const isCancelled = /cancel/i.test(msg) || /blocked/i.test(msg) || /closed/i.test(msg);
        if (isCancelled) {
          setShowFallback(true);
          toast.error("Login cancelado ou bloqueado. Use o botão abaixo para abrir em nova aba.");
        } else {
          toast.error(`Erro ao entrar com Google: ${msg}`);
        }
      }
    } catch (err: any) {
      const msg = err?.message || String(err);
      console.error("[Auth] Unexpected error:", msg, "| iframe:", isInIframe());
      setShowFallback(true);
      toast.error("Erro inesperado. Tente abrir o login em nova aba.");
    } finally {
      setLoginLoading(false);
    }
  };

  const handleOpenInNewTab = () => {
    window.open(window.location.origin + "/auth", "_blank");
  };

  if (authLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-[hsl(210_80%_12%)] flex items-center justify-center">
        <div className="animate-pulse text-muted-foreground">Carregando...</div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-[hsl(210_80%_12%)] flex items-center justify-center p-4">
      <Card className="w-full max-w-md border-0 bg-[hsl(210_80%_12%)] shadow-2xl shadow-black/40">
        <CardHeader className="space-y-1">
          <div className="flex flex-col items-center gap-6 mb-4">
            <Logo size="lg" />
            <div className="text-center">
              <CardTitle className="text-xl font-normal tracking-wide">Central Financeira</CardTitle>
            </div>
          </div>
          <CardDescription className="text-center">
            Entre com sua conta Google para acessar o sistema
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <Button
            type="button"
            variant="outline"
            className="w-full gap-2 h-12 text-base"
            onClick={handleGoogleLogin}
            disabled={loginLoading}
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            {loginLoading ? "Abrindo login..." : "Entrar com Google"}
          </Button>

          {showFallback && (
            <Button
              type="button"
              variant="secondary"
              className="w-full gap-2 h-12 text-base"
              onClick={handleOpenInNewTab}
            >
              <ExternalLink className="h-4 w-4" />
              Abrir login em nova aba
            </Button>
          )}
        </CardContent>
      </Card>
    </div>
  );
};

export default Auth;
