...
export const ReceivablesReport = () => {
  ...
  const items = data || [];
  const total = items.reduce((s, r: any) => s + Number(r.amount), 0);
  const maxVal = items.length ? Math.max(...items.map((r: any) => Number(r.amount))) : 0;
  ...
  const summaryCards = [
    ...
    { label: "Maior Valor", value: formatCurrency(maxVal), icon: ArrowUpRight, color: "text-primary" },
