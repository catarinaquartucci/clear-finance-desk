-- Corrigir a função aggregate_raw_to_detailed para ignorar coluna status
-- e determinar realizado/projetado baseado em payment_date vs CURRENT_DATE
...
BEGIN
  ...
  -- Realizado/Projetado é determinado por payment_date vs data atual (ignora coluna status)
  WITH aggregated AS (
    SELECT 
      ...
      -- REALIZADO: tem payment_date e data <= hoje
      SUM(CASE 
        WHEN mt.payment_date IS NOT NULL AND mt.payment_date::DATE <= CURRENT_DATE 
        THEN mt.movement_value 
        ELSE 0 
      END) as real_value,
      -- PROJETADO: não tem payment_date OU payment_date > hoje
      SUM(CASE 
        WHEN mt.payment_date IS NULL OR mt.payment_date::DATE > CURRENT_DATE 
        THEN mt.movement_value 
        ELSE 0 
      END) as proj_value
