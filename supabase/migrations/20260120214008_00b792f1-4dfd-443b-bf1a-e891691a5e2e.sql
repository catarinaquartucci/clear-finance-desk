-- Corrigir a função aggregate_raw_to_detailed para usar SEMPRE payment_date como base do mês
...
BEGIN
  -- Upsert agregando dados raw para detailed usando SEMPRE payment_date como referência de mês
  WITH aggregated AS (
    SELECT 
      TO_CHAR(mt.payment_date::DATE, 'YYYY-MM') as agg_month,
    ...
    WHERE mt.category_level_3_structure IS NOT NULL
      ...
      AND (p_year IS NULL OR EXTRACT(YEAR FROM mt.payment_date::DATE) = p_year)
    GROUP BY 
      TO_CHAR(mt.payment_date::DATE, 'YYYY-MM'),
