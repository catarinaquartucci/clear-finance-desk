...
// Shared Drive ID - arquivos serão salvos usando a quota do Shared Drive
...
async function createJWT(serviceAccount: ServiceAccountKey): Promise<string> {
  ...
  const claim = {
    ...
    scope: 'https://www.googleapis.com/auth/drive', // Full drive access for write
...
async function findFolder(accessToken: string, folderName: string, parentId: string): Promise<string | null> {
  ...
  // corpora=drive + driveId garante busca apenas no Shared Drive específico
...
// Create folder in Google Drive (supports Shared Drives)
...
async function uploadFile(
  ...
  // Added supportsAllDrives=true for Shared Drive support
...
serve(async (req) => {
  ...
  try {
    ...
    const { notaFiscalId, cnpj, nome, mesPagamento, arquivoUrls } = await req.json();
    
    console.log('Export request:', { notaFiscalId, cnpj, nome, mesPagamento, arquivoUrls });

    if (!notaFiscalId || !cnpj || !nome || !mesPagamento || !arquivoUrls || arquivoUrls.length === 0) {
      throw new Error('Missing required fields: notaFiscalId, cnpj, nome, mesPagamento, arquivoUrls');
    }
    ...
    // Check if month folder exists in Shared Drive, create if not
    let monthFolderId = await findFolder(accessToken, mesPagamento, SHARED_DRIVE_ID);
    
    if (!monthFolderId) {
      console.log(`Folder ${mesPagamento} not found in Shared Drive, creating...`);
      monthFolderId = await createFolder(accessToken, mesPagamento, SHARED_DRIVE_ID);
    } else {
      console.log(`Folder ${mesPagamento} found: ${monthFolderId}`);
    }
    ...
    for (let i = 0; i < arquivoUrls.length; i++) {
      ...
      const urlParts = arquivoUrl.split('/storage/v1/object/public/notas-fiscais/');
      ...
      // Generate filename: CNPJ_NOME_MM.YYYY_N.pdf (N is index if multiple files)
      const suffix = arquivoUrls.length > 1 ? `_${i + 1}` : '';
      const fileName = `${sanitizedCnpj}_${sanitizedNome}_${mesPagamento}${suffix}.pdf`;
      
      // Upload to Google Drive
      const fileId = await uploadFile(accessToken, fileContent, fileName, monthFolderId);
    ...
    console.error('Error in google-drive-export:', error);
    return new Response(
