...
serve(async (req) => {
  ...
  try {
    ...
    const callerIdP = claimsData.claims.sub;
    const { data: roleDataP } = await authSupabase
      .from('user_roles').select('role').eq('user_id', callerIdP).in('role', ['admin', 'finance']).limit(1).maybeSingle();
    if (!roleDataP) {
