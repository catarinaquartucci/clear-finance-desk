...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const callerIdCC = claimsData.claims.sub;
    const { data: roleDataCC } = await supabase
      .from('user_roles').select('role').eq('user_id', callerIdCC).in('role', ['admin', 'finance']).limit(1).maybeSingle();
    if (!roleDataCC) {
